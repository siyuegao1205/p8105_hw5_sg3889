...
the null is rejected increases as the true mean becomes larger,
...
-   Make a second plot showing the average of $\hat{\mu}$ only in
    samples for which the null is rejected on the y axis and the true
    value of $\mu$ on the x axis. Overlay it on the previous plot.
...
larger average estimate compared to the true value. As the effect size
increases, the power increases as well, and there are more rejected
samples where the $\hat{\mu}$ of these samples is significantly
different than 0 and closer to the true value of $\mu$. In this way,
it’s reasonable that the average of these rejected samples will be
