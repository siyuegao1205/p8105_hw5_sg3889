...
broom::tidy(baltimore_prop_test) %>% 
  select("Estimate Proportion" = estimate, "Lower 95% CI" = conf.low, "Upper 95% CI" = conf.high) %>% 
  knitr::kable(
    digits = 3
  )
```
...
As we can observe from the above scatterplot, the proportion of times the null is rejected increases as  the true mean becomes larger, indicating that the detectable effect size and power are positively correlated.
...
It can be explained by the fact that when the effect size is relatively small, the power is low, and fewer samples are rejected, leading to a larger average estimate compared to the true value. As the effect size increases, the power increases as well, and there are more rejected samples where the $\hat{\mu}$ of these samples is significantly different than 0 and closer to the true value of $\mu$. In this way, it's reasonable that the average of these rejected samples will be approximately equal to the true value of $\mu$.
